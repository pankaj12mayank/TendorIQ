...
from ...core.export.lite_policy import assert_lite_export_format, lite_export_pdf_only, lite_formats_payload
...
async def create_export(
    ...
    assert_lite_export_format(request.format)
...
async def export_proposal(
    ...
    assert_lite_export_format(format)
    user_id = current_user.user_id
    organization_id = _tenant_org_id(current_user)
    request = ExportRequest(
...
async def batch_export(
    ...
    if lite_export_pdf_only():
        raise HTTPException(status_code=400, detail='Batch export is not available in TenderIQ Lite')
    if len(exports) > 10:
    ...
    for export_req in exports:
        assert_lite_export_format(export_req.format)
...
async def create_secure_export(
    ...
    if lite_export_pdf_only():
        raise HTTPException(status_code=400, detail='Secure export is not available in TenderIQ Lite')
    assert_lite_export_format(request.format)
    if len(password) < 8:
