...
def permissions_for_role(role: str, membership_role: Optional[str]) -> list[str]:
    """Minimal permission strings for TenderIQ Lite."""
    if is_platform_super_admin(role):
        return ['all', *_LITE_PERMISSIONS]
    effective = normalize_membership_role(membership_role) or normalize_membership_role(role)
    if effective in ('owner', 'admin'):
        return list(_LITE_PERMISSIONS)
    if effective in ('manager', 'analyst'):
        return [p for p in _LITE_PERMISSIONS if p != 'billing:read'] + ['billing:read']
    return [p for p in _LITE_PERMISSIONS if ':read' in p or p.endswith(':run')]
