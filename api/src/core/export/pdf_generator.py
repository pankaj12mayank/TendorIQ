...
class PDFGenerator:
    ...
    def generate_custom_pdf(self, data: dict) -> bytes:
        ...
        if data.get('organization'):
            ...
            story.append(Paragraph(str(data['organization']), self._styles['Metadata']))

        meta_bits = []
        if data.get('status'):
            meta_bits.append(f"<b>Status:</b> {data['status']}")
        if data.get('created_at'):
            meta_bits.append(f"<b>Generated:</b> {data['created_at']}")
        if meta_bits:
            story.append(Spacer(1, 0.2 * cm))
            for item in meta_bits:
                story.append(Paragraph(item, self._styles['Metadata']))
