...
import { parseAnalysisDashboard } from '@/shared/analysis';
...
describe('analysis dashboard mapping', () => {
  ...
  it('maps importantClauses section', () => {
    const dashboard = parseAnalysisDashboard(
      {
        importantClauses: {
          clauses: [
    ...
    const ui = mapAnalysisDashboardToUi(dashboard);
    expect(ui.importantClauses.clauses).toHaveLength(1);
    expect(ui.importantClauses.clauses[0]?.title).toBe('Penalty clause');
  });
