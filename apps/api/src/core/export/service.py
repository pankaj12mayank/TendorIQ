...
class ExportService:
    ...
    def _get_source_data(self, source_type: str, source_id: str) -> Optional[dict]:
        ...
        from ..checklist.service import checklist_engine
        from ..risk_analysis.service import risk_analysis_engine
        ...
        try:
            if source_type == 'proposal':
                ...
                checklist = checklist_engine.get_checklist(source_id)
                if checklist:
                    from ..checklist.service import ChecklistExporter
                    exporter = ChecklistExporter(checklist)
                    return exporter.to_dict()
            elif source_type == 'risk_analysis':
                analysis = risk_analysis_engine.get_analysis(source_id)
                if analysis:
