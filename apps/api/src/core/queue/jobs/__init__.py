"""Job registry — handlers run in-process via core.tasks.inline."""

from typing import Any, Callable

from ..base_job import BaseJob, JobPriority, JobState, JobTracker, RetryHandler
...
async def ocr_batch(ctx: dict, document_ids: list[str], tenant_id: str, **kwargs) -> dict:
    from .ocr_jobs import OCRBatchJob

    job = OCRBatchJob(ctx, tenant_id=tenant_id)
    return await job.run(document_ids, tenant_id, kwargs)


async def parsing_process(
    ctx: dict, document_id: str, tenant_id: str, parsed_document_id: str, **kwargs
) -> dict:
    from .parsing_jobs import ParsingJob
...
async def parsing_batch(ctx: dict, parsed_document_ids: list[str], tenant_id: str, **kwargs) -> dict:
    from .parsing_jobs import ParsingBatchJob
...
async def chunking_process(
    ctx: dict, parsed_document_id: str, tenant_id: str, strategy: str = 'hybrid', **kwargs
) -> dict:
    from .parsing_jobs import ChunkingJob
...
async def document_analysis(
    ctx: dict, document_id: str, tenant_id: str, analysis_type: str = 'full', **kwargs
) -> dict:
...
async def batch_analysis(
    ctx: dict, document_ids: list[str], tenant_id: str, analysis_type: str = 'full', **kwargs
) -> dict:
    from .analysis_jobs import BatchAnalysisJob
...
async def email_send(ctx: dict, to_email: str, subject: str, body: str, **kwargs) -> dict:
    ...
    if queue_item_id:
        return await process_email_queue(ctx, queue_item_id, **kwargs)

    from .notification_jobs import EmailJob

    job = EmailJob(ctx)
    return await job.run(to_email, subject, body, **kwargs)


async def email_process(ctx: dict, queue_item_id: str, **kwargs) -> dict:
    from ...email.workers.email_worker import process_email_queue
...
async def notification_send(
    ...
    **kwargs,
    ...
    from .notification_jobs import NotificationJob
...
async def webhook_call(ctx: dict, url: str, method: str = 'POST', **kwargs) -> dict:
    from .notification_jobs import WebhookJob
...
async def scheduled_report(
    ctx: dict, report_id: str, tenant_id: str, recipients: list[str], format: str = 'pdf'
) -> dict:
    from .notification_jobs import ScheduledReportJob
...
async def export_document(
    ...
    **kwargs,
    ...
    return await job.run(
        export_id, export_type, format, source_id, source_type, user_id, organization_id, **kwargs
    )


async def batch_export(
    ctx: dict, batch_id: str, exports: list[dict], user_id: str, organization_id: str, **kwargs
) -> dict:
    from .export_jobs import BatchExportJob
...
async def scheduled_export(
    ...
    **kwargs,
    ...
    from .export_jobs import ScheduledExportJob

    job = ScheduledExportJob(ctx, tenant_id=organization_id)
    return await job.run(
        schedule_id, export_type, format, source_ids, recipients, user_id, organization_id, **kwargs
    )


async def export_cleanup(ctx: dict, older_than_hours: int = 24, **kwargs) -> dict:
    from .export_jobs import ExportCleanupJob
