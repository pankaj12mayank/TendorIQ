...
from .models import (
    Tenant, User, Membership, Tender, Bid, Document,
    AuditLog, Notification, Checklist, Risk, Proposal,
    PromptVersion, QueueJob, UsageLog, Subscription,
    AnalysisResult, TenantMixin
)
...
class TenantQueryHelper:
    ...
    @staticmethod
    async def count_tenant_bids(
        db: AsyncSession,
        tenant_id: UUID,
        status: Optional[str] = None
    ) -> int:
        """Count bids for tenant"""
        query = select(func.count(Bid.id)).where(Bid.tenant_id == tenant_id)
        if status:
    ...
    async def get_tenant_notifications(
        ...
        """Get notifications for tenant and user"""
        query = (
            select(Notification)
            .where(
                and_(
    ...
    async def get_tenant_prompt_versions(
        ...
        """Get prompt versions for tenant"""
        query = select(PromptVersion).where(PromptVersion.tenant_id == tenant_id)
        
        if prompt_type:
            query = query.where(PromptVersion.prompt_type == prompt_type)
        
        result = await db.execute(query.order_by(PromptVersion.version.desc()))
        return list(result.scalars().all())
